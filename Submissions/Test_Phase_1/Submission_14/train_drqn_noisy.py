"""
Training script for DRQN-Noisy (Deep Recurrent Q-Network with Noisy Networks) for OBELIX.

Example:
python train_drqn_noisy.py --obelix_py ../../../obelix.py --wall_obstacles

                    ALGORITHM: DEEP RECURRENT Q-NETWORK + NOISY NETWORKS (DRQN-NOISY)

DRQN-Noisy combines:
1. LSTM for temporal memory (like DRQN)
2. Noisy Networks for learned exploration (replaces epsilon-greedy)
3. Double DQN for reduced overestimation bias
...
Architecture:
    ...
    LSTM Layer (hidden_size=64)
        ↓
    Noisy Linear Layer (64 → 5 actions)

Key features:
- LSTM maintains hidden state across time steps within an episode
- Noisy Linear layers provide learned exploration (no epsilon-greedy needed)
