"""
DDQN with Noisy Networks for OBELIX submission.
"""
    ...
    for name in possible_names:
        ...
        if os.path.exists(path):
            try:
                ...
                _NETWORK = NoisyDQN(input_dim=18, hidden_dim=hidden_dim)
                _NETWORK.load_state_dict(checkpoint['q_network'])
                _NETWORK.eval()  # Disable noise for evaluation
                
                print(f"Loaded DDQN+Noisy from {path}")
                return _NETWORK
