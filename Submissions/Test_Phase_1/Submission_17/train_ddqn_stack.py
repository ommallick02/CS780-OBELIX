"""Train DDQN with State Stacking."""
...
@dataclass
class Transition:
    s: np.ndarray  # stacked, shape (18*STACK_SIZE,)
    a: int
    r: float
    s2: np.ndarray # stacked
    done: bool
...
def main():
    ...
    for ep in pbar:
        ...
        for _ in range(STACK_SIZE):
            frame_buf.append(np.zeros(18, dtype=np.float32))
        
        # Fill with first observation
        ...
        for _ in range(args.max_steps):
            ...
            if np.random.rand() < eps:
                ...
                with torch.no_grad():
                    s_t = torch.tensor(stacked, dtype=torch.float32, device=device).unsqueeze(0)
                    a = int(q(s_t).squeeze(0).argmax().item())

            s2, r, done = env.step(ACTIONS[a], render=False)
            ep_ret += float(r)
            
            # Update buffer and create next stacked obs
            frame_buf.append(s2.astype(np.float32))
            stacked_next = np.concatenate(list(frame_buf), axis=0)
            
            replay.add(Transition(s=stacked, a=a, r=float(r), s2=stacked_next, done=bool(done)))
        ...
        if avg_return > best_avg_return:
            ...
            q.to(device)
            pbar.set_postfix({"ret": f"{ep_ret:>8.1f}", "avg50": f"{avg_return:>8.1f}", "saved": "BEST"})
        else:
            pbar.set_postfix({"ret": f"{ep_ret:>8.1f}", "avg50": f"{avg_return:>8.1f}"})

    print(f"Training complete. Best avg50={best_avg_return:.1f} saved at {args.out}")
