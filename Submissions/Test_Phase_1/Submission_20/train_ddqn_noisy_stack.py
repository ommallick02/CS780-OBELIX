"""Train DDQN-Noisy with State Stacking."""
...
class NoisyLinear(nn.Module):
    ...
    def reset_noise(self):
        eps_in = self._scale(self.in_f)
        eps_out = self._scale(self.out_f)
...
class Replay:
    ...
    def sample(self, batch):
        ...
        return (torch.FloatTensor(np.array(s)), torch.LongTensor(a), torch.FloatTensor(r), 
                torch.FloatTensor(np.array(s2)), torch.FloatTensor(d))
...
def main():
    ...
    ap.add_argument("--out", default="ddqn_noisy_stack.pt")
    ap.add_argument("--episodes", type=int, default=5000)
    ap.add_argument("--difficulty", type=int, default=3)
    ap.add_argument("--wall_obstacles", action="store_true")
    ap.add_argument("--scaling_factor", type=int, default=5)  # Added this
    args = ap.parse_args()

    OBELIX = import_obelix(args.obelix_py)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    q = NoisyDQN().to(device)
    tgt = NoisyDQN().to(device)
    ...
    for ep in tqdm(range(args.episodes), desc="Noisy-Stack"):
