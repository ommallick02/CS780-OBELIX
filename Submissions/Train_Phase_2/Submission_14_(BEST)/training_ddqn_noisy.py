"""
Training script for DDQN with Noisy Networks for Exploration.
Replaces epsilon-greedy with learned noise in network weights.

python training_ddqn_noisy.py --obelix_py ./obelix.py --episodes 3000 --difficulty 2 --wall_obstacles
"""
...
class NoisyLinear(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, sigma_init: float = 0.017):
        ...
        # Non-learnable parameters (noise)
...
class DDQNNoisyAgent:
    """
    DDQN with Noisy Networks for exploration.
    No epsilon-greedy needed - exploration is learned via noise.
    """
    ...
    def __init__(
        ...
        # Networks
        self.q_network = NoisyDQN(input_dim, hidden_dim).to(self.device)
        self.target_network = NoisyDQN(input_dim, hidden_dim).to(self.device)
    ...
    def select_action(self, obs: np.ndarray) -> int:
        ...
        In training mode, noise provides exploration.
        In eval mode, noise is disabled (mean weights only).
        """
        ...
        # Greedy action (exploration comes from network noise)
    ...
    def save(self, path: str):
        ...
        print(f"Saved DDQN+Noisy to {path}")
...
def train(
    ...
    save_path: str = "ddqn_noisy_agent.pt",
    eval_interval: int = 50
):
    """Train DDQN with Noisy Networks."""
    ...
    agent = DDQNNoisyAgent()
    
    print(f"\n{'='*60}")
    print(f"Training DDQN + Noisy Networks")
    print(f"{'='*60}")
    ...
    with tqdm(total=episodes, desc="Training", unit="ep") as pbar:
        for episode in range(episodes):
            ...
            # Reset noise at episode start
            agent.reset_noise()
            
            for step in range(max_steps):
                # Select action (exploration via network noise)
                action_idx = agent.select_action(obs)
                action = ACTIONS[action_idx]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default="ddqn_noisy_agent.pt")
