"""
A2C (Advantage Actor-Critic) policy for OBELIX submission.
"""
    ...
    for name in possible_names:
        ...
        if os.path.exists(path):
            try:
                ...
                # Infer hidden dim
                first_weight = checkpoint['network_state_dict']['shared.0.weight']
                hidden_dim = first_weight.shape[0]
                
                _NETWORK = ActorCriticNetwork(input_dim=18, hidden_dim=hidden_dim)
                _NETWORK.load_state_dict(checkpoint['network_state_dict'])
                _NETWORK.eval()
                
                print(f"Loaded A2C network from {path}")
                return _NETWORK
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    """
    Select action using trained A2C policy.
    """
