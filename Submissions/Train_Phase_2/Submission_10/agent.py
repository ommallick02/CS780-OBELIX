"""
A3C (Asynchronous Advantage Actor-Critic) policy for OBELIX submission.
Uses the trained global network for inference.
"""
    ...
    for name in possible_names:
        ...
        if os.path.exists(path):
            try:
                ...
                print(f"Loaded A3C network from {path}")
                return _NETWORK
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    """
    Select action using trained A3C policy (greedy).
    """
